...
from ModelingMachine.engine.tasks.transformers import mmRDT2,mmUNIF
...
class TestmmUNIF(unittest.TestCase):
    ...
    def test_transform_after_RIDIT(self):
        ...
        for i in range(len(res2.colnames())):
            self.assertEqual(round(res2()[:,i].mean(),5),0.5)
        self.assertEqual(np.all(res2()>0),True)
        self.assertEqual(np.all(res2()<1),True)
        ...
        res2 = task2.transform(res)
        self.assertEqual(np.all(res2()<0.001),True)
    ...
    def test_transform_implementation(self):
        ...
        self.assertAlmostEqual(v[0,0], (0.914+1)/2, places=5)
        self.assertAlmostEqual(v[0,-1], (-0.222+1)/2, places=5)
        self.assertAlmostEqual(v[-1,0], (-0.43+1)/2, places=5)
        self.assertAlmostEqual(v[-1,-1], (0.974+1)/2, places=5)
