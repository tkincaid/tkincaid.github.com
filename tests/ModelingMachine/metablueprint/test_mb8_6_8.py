...
from ModelingMachine.metablueprint.mb8_6_8g import Metablueprint
import common.engine.metrics as metrics


class TestMB8_6_7(BaseTestMB):
    ...
    def test_reference_models_all_num_output_regression(self):
        ...
        bp_ids = [bpm.blueprint_id(model['blueprint'])
                  for model in ref_models]
        self.maxDiff = None
        expected_bp_ids = {'d4c06a5c23cf1d917019720bceba32c8',
    ...
    def test_reference_models_all_cat_output_regression(self):
        ...
        bp_ids = [bpm.blueprint_id(model['blueprint'])
                  for model in ref_models]
        expected_bp_ids = {'3010cf2f2d8f7e64f2665c0223ae6bd7',
    ...
    def test_reference_models_num_and_cat_output_regression(self):
        ...
        bp_ids = [bpm.blueprint_id(model['blueprint'])
                  for model in ref_models]
        self.maxDiff = None
        expected_bp_ids = {"08f15f00ed64dfd34fbae67b442228e5",
    ...
    def test_just_default_insight_models_when_only_text_available(self):
        ...
        with patch.object(mb, 'get_metric_for_models') as fake_m:
            ...
            self.assertEqual(bpm.blueprint_id(models[1]['blueprint']),
                             '34ce2c66e6fab69e871f01e640c0d725')
            self.assertEqual(bpm.blueprint_id(models[2]['blueprint']),
                             'd7845e402515277924af4ff9e3d1a5e2')
            self.assertEqual(bpm.blueprint_id(models[3]['blueprint']),
                             '3e84b36556ad33a954a9e2f2e39d7f3a')

    def test_modeling_parameters_knows_weights(self):
        pid = self.make_fake_project(varTypeString='NNYW',
                                     task_type='Binary',
                                     weight_col_name='weight')
        mb = Metablueprint(pid, None)
        with patch.object(mb, 'get_metric_for_models') as fake_m:
            params = mb.get_modeling_parameters()
            self.assertIsNotNone(params.weighted_columns)
            self.assertEqual(params.weighted_columns['weight'], 'weight')

    def test_ams_15_maps_to_gini_norm(self):
        pid = self.make_fake_project(varTypeString='NNYN',
                                     task_type='Binary')
        mb = Metablueprint(pid, None)
        grid_metric = mb.get_metric_for_grid_search(metrics.AMS_15)
        self.assertEqual(grid_metric, metrics.GINI_NORM_W)

    def test_ams_opt_maps_to_gini_norm(self):
        pid = self.make_fake_project(varTypeString='NNYN',
                                     task_type='Binary')
        mb = Metablueprint(pid, None)
        grid_metric = mb.get_metric_for_grid_search(metrics.AMS_OPT)
        self.assertEqual(grid_metric, metrics.GINI_NORM_W)